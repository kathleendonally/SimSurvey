...
strat_means <- function(data = NULL, metric = NULL, strat_groups = NULL,
                        survey_groups = NULL, confidence = 95, between_tow_var = FALSE) {
  ...
  if (!between_tow_var) {
    ...
    # BTV: Use mean tow_area within each strat
    d <- copy(data)
    required_cols <- unique(c(strat_groups, metric, "tow_area", "strat_area"))
    d <- d[, ..required_cols]
    setnames(d, metric, "metric_col")  # we renamed the column to "metric_col"

    strat_tab <- d[, .(
      sumYh = sum(metric_col),
      meanYh = mean(metric_col),
      varYh = stats::var(metric_col),
      nh = .N,
      tow_area_mean = mean(tow_area),
      strat_area = unique(strat_area)
    ), by = strat_groups]

    strat_tab[, Nh := strat_area / tow_area_mean]
    strat_tab[, Wh := Nh / sum(Nh), by = survey_groups]
    strat_tab[, total := Nh * sumYh / nh]
    strat_tab[, gh := Nh * (Nh - nh) / nh]
  }
...
run_strat <- function(sim,
  ...
  strat_args <- list(
    data = data$setdet,
    metric = "n",
    strat_groups = if (between_tow_var) {
      c("sim", "year", "division", "strat", "strat_area")
    } else {
      c("sim", "year", "division", "strat", "strat_area", "tow_area")
    },
